...
public static class DownloaderResolver
{
    ...
    private static IEnumerable<IAsyncDownloader> GetDownloaderSubTypesDynamically()
    {
        ...
        IEnumerable<Type>? subtypeInfoElems = types.Where(x => x.GetInterface(nameof(IAsyncDownloader)) != null);
        int subtypeInfoCount = subtypeInfoElems.Count();

        IAsyncDownloader[]? subtypeObjs = new IAsyncDownloader[subtypeInfoCount];
        for (int i = 0; i < subtypeInfoCount; i++)
        {
            Type? subtypeInfo = subtypeInfoElems.ElementAt(i);
            IAsyncDownloader? newObj = (IAsyncDownloader)(asm.CreateInstance(subtypeInfo.FullName ?? throw new Exception("Full name for downloader type was null.")) ?? throw new Exception("Could not create downloader type dynamically."));
