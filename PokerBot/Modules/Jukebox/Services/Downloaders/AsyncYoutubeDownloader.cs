...
namespace PokerBot.Modules.Jukebox.Services.Downloaders
{
    public class AsyncYoutubeDownloader : IAsyncDownloadService
    {
        ...
        private async Task<PlayableMedia> CacheAsync(IAsyncMediaCache cache, PlayableMedia toCache, string cacheName, bool checkCacheSize, int attempt = 0)
        {
            ...
            if (result == null)
            {
                ...
                await CacheAsync(cache, toCache, cacheName, checkCacheSize, ++attempt);
            }
            return result;
        }

        public async Task<MediaCollection> DownloadAsync(IAsyncMediaCache cache, string searchQuery, bool checkCacheSize = true, Action largeSizeWarningCallback = null, Action<string> videoUnavailableCallback = null)
        {
            if (YoutubeClient.TryParsePlaylistId(searchQuery, out var playlistId))
            {
                ...
                Parallel.For(0, videos.Count, i =>
                {
                    ...
                    if (result == null)
                        return;
                    med[i] = (CacheAsync(cache, result, result.Title, false).Result);
                });

                return new MediaCollection(med.ToArray(), pl.Title, (await Utility.Utility.GetURLArgumentValueAsync<int?>(searchQuery, "index", false)) ?? 1);
            }
